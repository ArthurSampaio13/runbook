...
class AWSRunbookGenerator:
    def __init__(self):
        ...
        self.cross_account_role = os.getenv('CROSS_ACCOUNT_ROLE', 'OrganizationAccountAccessRole')
    ...
    def _get_target_accounts(self):
        ...
        try:
            ...
            accounts = [acc['Id'] for acc in response['Accounts'] if acc['Status'] == 'ACTIVE']
    ...
    def run_collection(self, account_id, region):
        ...
        try:
            ...
            if result.returncode != 0:
                logger.error(f"Collection failed for {account_id}/{region}: {result.stderr}")
                return None
...
"""
    ...
    def convert_to_docx(self, md_file):
        ...
        try:
            try:
                ...
                logger.error("Pandoc command not found. Please install pandoc and ensure it's in your PATH.")
                return None
            except subprocess.CalledProcessError as e:
                logger.error(f"Pandoc --version check failed: {e.stderr or e.stdout or str(e)}. Ensure pandoc is correctly installed.")
                return None
            ...
            subprocess.run(pandoc_conversion_cmd, capture_output=True, text=True, check=True)
            logger.info(f"DOCX file generated: {docx_file}")
            return docx_file
        except subprocess.CalledProcessError as e:
            logger.error(f"Pandoc conversion failed for '{md_file}'. Error: {e.stderr.strip() if e.stderr else (e.stdout.strip() if e.stdout else str(e))}")
            return None
    ...
    def _get_reference_doc(self):
        ...
        if ref_doc.exists():
            ...
            logger.info(f"Reference DOCX 'reference.docx' not found in {self.output_dir}. Pandoc will use default styling.")
            return None
    def run(self):
        ...
        logger.info(f"Successfully processed {len(results)} account/region combinations")
